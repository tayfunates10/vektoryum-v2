...
void test_image_spec() {
    ...
    invalid.width = 0U;
    expect_true(
        validate_image_spec(invalid).error == ImageSpecError::ZeroWidth,
        "zero width rejected");
    ...
    expect_true(
        validate_image_spec(invalid).error == ImageSpecError::AlphaModeWithoutAlphaChannel,
        "alpha mode without alpha channel rejected");

    invalid = rgba16;
    invalid.alpha = AlphaMode::None;
    expect_true(
        validate_image_spec(invalid).error == ImageSpecError::MissingAlphaMode,
        "alpha-capable layout requires alpha mode");

    const ImageSpec huge{
    ...
    expect_true(
        validate_image_spec(huge).error == ImageSpecError::SizeOverflow,
...
void test_color_and_alpha() {
    ...
    for (const double value : {0.0, 0.003, 0.018, 0.18, 0.5, 0.75, 1.0}) {
        const double round_trip = linear_to_srgb(srgb_to_linear(value));
        expect_near(round_trip, value, 1e-12, "sRGB linear round-trip is stable");
    }

    const Rgba64 straight{0.8, 0.4, 0.2, 0.25};
    const Rgba64 premultiplied = premultiply_alpha(straight);
    expect_near(premultiplied.r, 0.2, 1e-12, "premultiply red");
    expect_near(premultiplied.g, 0.1, 1e-12, "premultiply green");
    expect_near(premultiplied.b, 0.05, 1e-12, "premultiply blue");
    expect_near(premultiplied.a, 0.25, 1e-12, "premultiply alpha preserved");
    ...
    expect_true(
        transparent.r == 0.0 && transparent.g == 0.0 && transparent.b == 0.0 && transparent.a == 0.0,
...
void test_tiles() {
    ...
    for (const TileRegion& tile : plan.tiles) {
        ...
        const std::uint64_t expanded_right =
            static_cast<std::uint64_t>(tile.expanded.x) + tile.expanded.width;
        const std::uint64_t expanded_bottom =
            static_cast<std::uint64_t>(tile.expanded.y) + tile.expanded.height;
    ...
    const TileRegion& first = plan.tiles[0];
    const TileRegion& second = plan.tiles[1];
    const std::uint32_t first_expanded_right = first.expanded.x + first.expanded.width;
    expect_true(
        first_expanded_right > second.expanded.x,
        "adjacent expanded tiles overlap for seam-safe processing");

    const TilePlan one = plan_tiles(64U, 64U, 256U, 32U);
    expect_true(one.ok() && one.tiles.size() == 1U, "small image uses one bounded tile");
    expect_true(
        one.tiles[0].expanded.x == 0U && one.tiles[0].expanded.y == 0U &&
            one.tiles[0].expanded.width == 64U && one.tiles[0].expanded.height == 64U,
        "single tile clips overlap to image bounds");

    expect_true(
        plan_tiles(0U, 64U, 32U, 4U).error == TilePlanError::ZeroImageDimension,
        "zero image dimension rejected by tile planner");
    expect_true(
        plan_tiles(64U, 64U, 0U, 4U).error == TilePlanError::ZeroTileExtent,
        "zero tile extent rejected");
    expect_true(
        plan_tiles(1000U, 750U, 1U, 0U, 10U).error == TilePlanError::TooManyTiles,
        "tile-count resource budget enforced");
}

void test_decode_limits() {
    using namespace vektoryum;

    const core::ImageSpec image{
        1024U,
        1024U,
        core::PixelLayout::RGBA,
        core::ChannelType::UInt8,
        core::TransferFunction::SRGB,
        core::ColorPrimaries::SRGB,
    ...
    io::DecodeLimits width_limited{};
    width_limited.max_width = 512U;
    expect_true(
        io::validate_decode_limits(image, width_limited).error == io::DecodeLimitError::WidthExceeded,
        "decode width limit enforced");

    io::DecodeLimits pixel_limited{};
    pixel_limited.max_pixels = 1'000'000U;
    expect_true(
        io::validate_decode_limits(image, pixel_limited).error == io::DecodeLimitError::PixelCountExceeded,
        "decode pixel budget enforced");

    io::DecodeLimits byte_limited{};
    byte_limited.max_decoded_bytes = 1U * 1024U * 1024U;
    expect_true(
        io::validate_decode_limits(image, byte_limited).error == io::DecodeLimitError::ByteBudgetExceeded,
        "decode byte budget enforced");
}
