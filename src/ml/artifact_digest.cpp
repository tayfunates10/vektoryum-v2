...
std::string sha256_hex(const std::vector<std::uint8_t>& bytes) {
    ...
    for (int shift = 56; shift >= 0; shift -= 8) {
        message.push_back(static_cast<std::uint8_t>((bit_length >> static_cast<unsigned>(shift)) & 0xffULL));
    }
    ...
    for (std::size_t offset = 0U; offset < message.size(); offset += 64U) {
        ...
        for (std::size_t index = 0U; index < 64U; ++index) {
            ...
            const auto temp1 = h + s1 + choice + kSha256RoundConstants[index] + words[index];
