...
The mixed-content execution fixture exercises the existing contracts together on a deterministic vector+raster plan. It covers overlapping seam pixels, a tiny vector-owned feature under transparent raster coverage, alternating high-frequency raster detail, repeated alpha-safe execution equality, final seam/topology validation and canonical end-to-end provenance evidence containing both source identities. The fixture is a standalone CTest target and is included in sanitizer builds so the execution path is exercised on every supported CI platform without weakening any existing gate.

## Remaining acceptance

No material Stage 9 contract gap is known after the mixed-content execution fixture. Exact-head cross-platform and sanitizer CI must validate this final slice before Stage 9 can be completed and merged.
