...
The sixth slice replaces the Stage 12 synthetic certificate fixture at the public API boundary with a certificate produced by the real Stage 11 issuance path. The test builds a canonical Stage 10 export artifact, calls `issue_quality_certificate(...)`, proves repeated issuance returns byte-identical canonical certificate bytes and the same certificate digest, and then hands that exact artifact into `validate_certified_operation_request(...)`. The public API accepts both independently issued but byte-identical artifacts, while request-digest substitution, artifact-digest substitution, canonical-byte tampering, malformed digest data and operation substitution remain fail-closed. This regression stays in the existing Stage 12 CTest target and therefore receives Ubuntu, Windows, macOS and Linux ASan/UBSan coverage without weakening any earlier gate.

The seventh slice adds deterministic certified processing execution at the stable Core API boundary. `execute_certified_operation(...)` first reuses the exact Stage 12 certificate validator and then returns a canonical certified response that carries the validated Stage 11 certificate digest only on success. Repeated execution with independently issued but byte-identical Stage 11 certificates produces byte-identical response bytes. Provenance substitution remains fail-closed with data exit semantics, and failed responses deliberately carry no certificate digest so rejected evidence cannot be reflected as accepted provenance. The existing Stage 12 CTest target covers this path on Ubuntu, Windows, macOS and Linux ASan/UBSan.

The eighth slice completes the certified processing CLI transport. `--certified-export REQUEST_ID [CERTIFICATE_SHA256]` creates the same deterministic Stage 10 export and real Stage 11 certificate fixture used to prove issuance-to-command handoff, routes the request through `execute_certified_operation(...)`, and writes `canonical_certified_response_report(...)` directly to stdout. Omitting the optional digest binds the exact issued certificate identity; supplying that exact digest must produce byte-identical response bytes, while a substituted digest fails closed with exit code 65, the canonical Core API error response, and an empty reflected certificate identity. The byte-exact transport regression runs on Ubuntu, Windows and macOS and again against the Linux ASan/UBSan binary. No alternate response serializer or CLI-specific provenance acceptance path exists.
