...
- Add the final CLI transport for the certified processing path so the public command boundary emits the same canonical certified response bytes as the Core API while preserving the exact issued Stage 11 certificate identity and rejecting alternate serialization/substitution.
