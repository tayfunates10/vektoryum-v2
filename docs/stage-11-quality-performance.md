...
A dedicated Stage 11 end-to-end certification regression now composes only measured evidence: validated Stage 10 canonical exports are measured for resource/performance evidence, bounded canonical comparison fixtures are measured for alpha/vector quality, the resulting strictly ordered metric set is issued as the deterministic certificate artifact, and repeated execution must produce identical evidence and certificate bytes/digest. The regression runs independently for canonical SVG, PDF, EPS and DXF exports, rejects output-provenance substitution, rejects tampered Stage 10 artifact bytes before measurement or issuance, and rejects threshold-violating measured image quality. Repeatability coverage also includes an exact-boundary fixture with `alpha_mae == 0.02` and `vector_iou == 0.99`; one-step excursions beyond either immutable threshold fail closed. The target is part of normal CTest and Linux ASan/UBSan coverage, so this chain cannot bypass the existing Stage 0-10 gates.

## Remaining acceptance

Implementation work is complete for the documented Stage 11 contracts. Final acceptance now requires a fresh exact-head run with source hygiene, Ubuntu, Windows, macOS and Linux ASan/UBSan fully green, mergeable=true, and zero unresolved blocking review threads before Stage 11 may be merged or counted complete.
