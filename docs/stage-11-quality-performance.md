...
A dedicated Stage 11 end-to-end certification regression now composes only measured evidence: validated Stage 10 canonical exports are measured for resource/performance evidence, bounded canonical comparison fixtures are measured for alpha/vector quality, the resulting strictly ordered metric set is issued as the deterministic certificate artifact, and repeated execution must produce identical evidence and certificate bytes/digest. The regression runs independently for canonical SVG and PDF exports, rejects output-provenance substitution, rejects tampered Stage 10 artifact bytes before measurement or issuance, and rejects threshold-violating measured image quality. The target is part of normal CTest and Linux ASan/UBSan coverage, so this chain cannot bypass the existing Stage 0-10 gates.

## Remaining acceptance

Material work remains: broaden repeatability across the remaining canonical Stage 10 formats and additional near-threshold quality fixtures, then perform final exact-head Stage 11 acceptance with all required CI and review/mergeability gates.
